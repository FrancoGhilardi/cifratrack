 ...
 */
export const createInvestmentSchema = investmentBaseSchema.superRefine(
  (data, ctx) => {
    // Validar duración
 ...
 */
export const updateInvestmentSchema = investmentBaseSchema
  .partial()
  .superRefine((data, ctx) => {
    // Validar duración solo si se proporciona
    ...
    // Validar consistencia isCompound/days si ambos están presentes
    if (data.isCompound !== undefined && data.days !== undefined) {
      if (!data.isCompound && data.days === 0) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: "La duración debe ser mayor a cero días",
