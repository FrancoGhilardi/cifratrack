import { pgTable, unique, uuid, varchar, timestamp, text, char, foreignKey, bigint, boolean, index, check, numeric, smallint, date, integer, primaryKey, pgEnum } from "drizzle-orm/pg-core"
...
export const categories = pgTable("categories", {
	...
	unique("categories_user_id_kind_name_key").on(table.userId, table.kind, table.name),
]);
...
export const transactions = pgTable("transactions", {
	...
	index("idx_tx_user_month").using("btree", table.userId.asc().nullsLast().op("uuid_ops"), table.occurredMonth.asc().nullsLast().op("date_ops")),
	index("idx_tx_user_status").using("btree", table.userId.asc().nullsLast().op("uuid_ops"), table.status.asc().nullsLast().op("enum_ops")),
	foreignKey({
...
export const investments = pgTable("investments", {
	...
	startedOn: date("started_on").default(sql`CURRENT_DATE`).notNull(),
	notes: text(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("idx_investments_user").using("btree", table.userId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [users.id],
	...
	check("investments_tna_check", sql`(tna >= (0)::numeric) AND (tna <= 999.99)`),
]);
