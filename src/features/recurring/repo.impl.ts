...
export class RecurringRuleRepository implements IRecurringRuleRepository {
  ...
  async update(id: string, userId: string, data: UpdateRecurringRuleInput): Promise<RecurringRule> {
    ...
    const [row] = await db
      ...
      .set({
        ...
        amount: data.amount !== undefined ? Math.trunc(data.amount) : existing.amount,
        kind: data.kind ?? existing.kind,
        dayOfMonth: data.dayOfMonth ?? existing.dayOfMonth,
        status: data.status ?? existing.status,
        paymentMethodId: data.paymentMethodId ?? existing.paymentMethodId,
        activeFromMonth: data.activeFromMonth ?? existing.activeFromMonth.toString(),
        activeToMonth: data.activeToMonth ?? existing.activeToMonth?.toString() ?? null,
        updatedAt: now,
      })
      .where(and(eq(recurringRules.id, id), eq(recurringRules.userId, userId)))
      .returning();
  ...
  async findCategories(ruleId: string): Promise<Array<{ categoryId: string; allocatedAmount: number }>> {
    const rows = await db
      .select({
    ...
    return rows.map((row) => ({ categoryId: row.categoryId, allocatedAmount: Number(row.allocatedAmount) }));
  }
  ...
  async setCategories(
    ...
    await db.delete(recurringRuleCategories).where(eq(recurringRuleCategories.recurringRuleId, ruleId));
  ...
  async createTransactionFromRule(rule: RecurringRule, month: Month, splits: Array<{ categoryId: string; allocatedAmount: number }>) {
    const occurredOn = `${month.toString()}-${rule.dayOfMonth.toString().padStart(2, '0')}`;
