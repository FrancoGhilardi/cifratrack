...
export class InvestmentRepository implements IInvestmentRepository {
   ...
   */
  async list(userId: string, params: InvestmentQueryParams): Promise<PaginatedInvestments> {
    const {
      page = 1,
      pageSize = 20,
      sortBy = 'startedOn',
    ...
    // Validar parámetros
    const validSortBy = ['startedOn', 'principal', 'tna', 'days', 'platform', 'title'];
    ...
    if (active === 'true') {
      // Inversión activa: startedOn + days > hoy
      conditions.push(
        sql`${investments.startedOn} + INTERVAL '1 day' * ${investments.days} > CURRENT_DATE`
      );
    } else if (active === 'false') {
      // Inversión finalizada: startedOn + days <= hoy
      conditions.push(
        sql`${investments.startedOn} + INTERVAL '1 day' * ${investments.days} <= CURRENT_DATE`
      );
    }
    ...
    const getOrderColumn = () => {
      switch (sortBy) {
        case 'startedOn': return investments.startedOn;
        case 'principal': return investments.principal;
        case 'tna': return investments.tna;
        case 'days': return investments.days;
        case 'platform': return investments.platform;
        case 'title': return investments.title;
        default: return investments.startedOn;
      }
    };
  ...
  async update(id: string, userId: string, data: Partial<Investment>): Promise<Investment> {
    ...
    if (data.principal !== undefined) updateData.principal = data.principal.toString();
