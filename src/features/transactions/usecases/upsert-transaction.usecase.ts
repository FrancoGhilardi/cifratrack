...
export class UpsertTransactionUseCase {
  ...
  async create(
    ...
    if (data.split && data.split.length > 0) {
      ...
      throw new ValidationError(
        "Debe especificar al menos una categoría en split"
      );
    }
  ...
  async update(
    ...
    if (data.split && data.split.length > 0) {
      ...
      if (!amount) {
        throw new ValidationError(
          "No se puede determinar el monto para validar split"
        );
      }
  ...
  private validateSplits(
    ...
    if (invalidSplit) {
      throw new ValidationError(
        "Los importes de categorías deben ser mayores a cero"
      );
    }
    ...
    if (categoryIds.length !== uniqueIds.size) {
      throw new ValidationError(
        "No puede asignar la misma categoría más de una vez"
      );
    }
  }
}
